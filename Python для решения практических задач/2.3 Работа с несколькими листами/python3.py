...
def main():
    """My function."""
    workbook = open_workbook("trekking3.xlsx")
    value = nutritional_value(workbook.sheet_by_name("Справочник"))
    sheet_2 = workbook.sheet_by_name("Раскладка")
    days = sheet_2.col_values(0)[1:]
    products, masses = sheet_2.col_values(1)[1:], sheet_2.col_values(2)[1:]
    plan = {}
    for day, food, weight in zip(days, products, masses):
        plan.setdefault(day, []).append((food, weight))
    for product_mass in plan.values():
        print(*(int(sum((value[product][j] or 0) * mass
                        for product, mass in product_mass) // 100)
                for j in range(4)))
